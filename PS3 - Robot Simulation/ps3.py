...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        """
        # Check that variables are within limits
        if not(width > 0 and height > 0 and dirt_amount > 0):
            raise ValueError("One or more parameters are invalid!")

        # Initialise the variables for a rectangular room:
        self.width = width 
        self.height = height

        # Create a dictionary with all tiles as keys, set with uniform dirt_amount to start
        self.tiles = {}
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        # Initialise variable for cleaned number of tiles
        clean = 0
    ...
    def is_position_in_room(self, pos):
        ...
        """
        # Obtain x and y of position
        pos_x, pos_y = math.floor(pos.get_x()), math.floor(pos.get_y())
        # Check if within range of room
        if (pos_x, pos_y) in self.tiles.keys():
            return True
        else: 
            return False
...
class FurnishedRoom(RectangularRoom):
    ...
    def add_furniture_to_room(self):
        ...
        for x in range(f_bottom_left_x, f_bottom_left_x + furniture_width):
            for y in range(f_bottom_left_y, f_bottom_left_y + furniture_height):
                self.furniture_tiles.append((x,y))             
    ...
    def is_position_furnished(self, pos):
        ...
        """
        pos_x, pos_y = math.floor(pos.get_x()), math.floor(pos.get_y())
        return self.is_tile_furnished(pos_x, pos_y)
    ...
    def get_random_position(self):
        ...
        while True:
            ...
            rand_pos_x, rand_pos_y = random.uniform(0.0, self.width), random.uniform(0.0, self.height)
            rand_pos =  Position(rand_pos_x, rand_pos_y)
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        # Check if Robot is faulty
        if self.gets_faulty():
        # If it is, change direction randomly
            self.set_robot_direction(random.uniform(0,360))
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                FaultyRobot)
    """
    # Initialise average time to clean the min_coverage amount
    avg_time = 0
    ...
    for trial in range(num_trials):
        ...
        # Initialise the time step for each trial
        time_step = 0
        current_coverage = room.get_num_cleaned_tiles() / room.get_num_tiles()
        # Keep Robots cleaning until min_coverage achieved
        while current_coverage < min_coverage:
            # Update the Robot position for every time step
            for robot in robots:
        ...
        avg_time = ((avg_time * trial) + time_step) / (trial+1)
    return avg_time
...
#   of a 20x20 room?
#   The relationship between the time and amount of rovers appears to be an exponential decay, with the more 
#   Robots cleaning at one given time, the faster the cleaning time on average. When considering Faulty Robots
#   They are overall worse in performance as they take longer to clean the room on average but as more Robots
#   are considered, the affect of the drop in performance decreases as it starts to match the time taken by the
#   Standard Robots.
#
# 2) How does the performance of the two robot types compare when two of each
#    robot cleans 80% of rooms with dimensions 
#    10x30, 20x15, 25x12, and 50x6?
#    The overall relationship is that after an aspect ratio of about 1.3 
#    (the minimum), there is an increase in time taken to clean to 80%
#    as the aspect ratio increases but the rate of increase is much greater
#    with Faulty Robots.
